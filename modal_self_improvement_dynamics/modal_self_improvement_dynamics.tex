...
Modal Takeoff Kernels for Solver--Verifier Dynamics}
...
\begin{abstract}
Recent phenomenological theory models language-model self-improvement by a
solver--verifier uncertainty gap whose linearized dynamics converge with one
exponential rate.  We show that, after sampling at training checkpoints, this
law is exactly a rank-one takeoff kernel.  The reduction yields joint empirical
restrictions stronger than separate exponential fits: normalized solver,
verifier, and gap trajectories must share one discrete mode, consecutive kernel
ratios must be constant, and all second-order Hankel minors must vanish.  We
then give the minimal finite-dimensional extension.  A locally linear hidden
state produces a finite modal kernel whose modes are the active eigenvalues of
the sampled transition operator.  The active modal radius determines settling
time, while the exact kernel Hankel rank equals the number of distinct active
modes and a matrix pencil recovers them.  A constructive separation then gives
rank-one and rank-two trajectories with the same endpoints but arbitrarily
different settling times.  These results separate the terminal amount of
improvement from its response shape and give a direct route from current
self-improvement theory to falsifiable modal diagnostics.  A calibrated
synthetic power study further shows that Hankel tests on the residual can be
substantially more noise-tolerant than tests on its differenced kernel.  When
the asymptote is estimated jointly, held-out model selection can detect rank-one
misspecification well before the endpoint or individual modes are reliably
recovered.  An exploratory extraction of the published Phi-4 vector
trajectories finds held-out departures from both separate rank-one fits and the
stronger shared-mode restriction.  A three-seed resource-scaled reproduction
narrows the per-token gap qualitatively but does not yield stable modes.  In a
five-boundary audit, power laws win $27/60$ published-curve forecasts whereas
persistence wins $52/90$ reproduction forecasts; no published curve keeps one
winner across all boundaries.  An endpoint-free matrix-pencil audit recovers
both poles in only $12.5\%$ of matched rank-two controls at this resolution,
yet stable one-pole summaries differ systematically across the published gap,
solver, and verifier curves.  Thus the framework rejects one exact shared rate
while leaving modal count unidentified; it does not claim identified
mechanisms.
\end{abstract}
...
Our first contribution is an exact reduction: the current linearized
solver--verifier law is the one-mode member of a larger family.  Our second is
the minimal generalization: a finite-dimensional local state produces a finite
mixture of response modes.  We then connect those modes to settling time and
Hankel rank, which supplies an exact inverse procedure in the noiseless case.
At this stage we make no empirical claim that more than one mode is required.
...
\section{Exploratory Published-Trajectory Audit}
...
\section{Fresh-Seed Reproduction}
...
\section{Rolling Baseline Audit}
...
\begin{enumerate}
 ...
 \item compare against regularized two-mode and nonparametric alternatives;
