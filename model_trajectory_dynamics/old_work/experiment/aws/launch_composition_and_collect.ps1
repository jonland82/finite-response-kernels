...
$runId = "composition-seed$Seed-" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
$runRoot = Join-Path $experimentRoot "runs\$runId"
New-Item -ItemType Directory -Path $runRoot -Force | Out-Null

$account = aws sts get-caller-identity --query Account --output text
if ($LASTEXITCODE -ne 0) { throw "AWS identity lookup failed" }
$bucket = "causal-composition-$account-" + $runId.Replace("composition-", "").ToLowerInvariant()
...
try {
    ...
    tar -czf $sourceArchive -C $experimentRoot run_experiment.py run_causal_composition.py
    ...
    $getUrl = aws s3 presign "s3://$bucket/$sourceKey" --region $Region --expires-in 3600
    $putUrl = python $presignPath --bucket $bucket --key $resultKey --region $Region --expires 3600
...
#!/bin/bash
set -uo pipefail
mkdir -p /opt/causal-composition/results
exec > >(tee -a /var/log/causal-composition.log) 2>&1
...
tar -xzf /tmp/source.tar.gz -C /opt/causal-composition

dnf install -y python3.11 python3.11-pip
python3.11 -m venv /opt/causal-composition/venv
/opt/causal-composition/venv/bin/pip install --upgrade pip
/opt/causal-composition/venv/bin/pip install --index-url https://download.pytorch.org/whl/cpu torch==2.10.0
/opt/causal-composition/venv/bin/pip install numpy matplotlib
PYTHON_BIN=/opt/causal-composition/venv/bin/python
...
if [ ! -x "$PYTHON_BIN" ] || ! "$PYTHON_BIN" -c 'import torch, numpy, matplotlib' >/dev/null 2>&1; then
  ...
  timeout --signal=INT 1500 "$PYTHON_BIN" /opt/causal-composition/run_causal_composition.py \
    --output-dir /opt/causal-composition/results \
    --max-runtime-seconds 1200 \
    --seed __SEED__
  RUN_EXIT=$?
fi

echo "run_exit_code=$RUN_EXIT" | tee /opt/causal-composition/results/cloud_exit_status.txt
cp /var/log/causal-composition.log /opt/causal-composition/results/instance_console.log
tar -czf /tmp/causal-composition-results.tar.gz -C /opt/causal-composition results
curl --fail --retry 5 --retry-delay 3 \
  -H 'Content-Type: application/gzip' \
  --upload-file /tmp/causal-composition-results.tar.gz \
  '__PUT_URL__'
UPLOAD_EXIT=$?
...
'@
    ...
    $userData = $userData.Replace("__SEED__", [string]$Seed)
    ...
    $securityGroupId = aws ec2 create-security-group `
        --group-name "causal-composition-$runId" `
        --description "No-ingress group for capped causal composition pilot" `
        --vpc-id $vpcId `
        --region $Region `
        --query GroupId `
        --output text
    ...
    $tagSpec = "ResourceType=instance,Tags=[{Key=Name,Value=causal-composition-pilot},{Key=Project,Value=model-trajectory-dynamics},{Key=RunId,Value=$runId},{Key=AutoTerminateMinutes,Value=$HardStopMinutes}]"
