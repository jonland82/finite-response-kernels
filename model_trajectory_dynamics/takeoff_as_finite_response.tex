...
sampling.
The same need to resolve local timing now appears at practical scale: one-pass
mixture-of-experts pretraining has shown asynchronous memorization-to-generalization
transitions across data groups \cite{li2026}.
...
circuit-level cause.
More generally, delayed generalization has been attributed to a transition from
lazy to rich feature learning \cite{kumar2024} and, in homogeneous networks, to
distinct early- and late-phase implicit biases \cite{lyu2024}.
...
The optimizer and regularization choice is substantive: numerical stability and
gradients that scale logits without changing predictions can alter or prevent
grokking \cite{prieto2025}.
...
\begin{questionbox}
\noindent\textcolor{questionink}{\bfseries Conceptual takeaways}
\begin{itemize}[
  leftmargin=1.45em,
  label=\textcolor{questionline}{\(\bullet\)},
  itemsep=0.35em,
  topsep=0.35em,
  parsep=0pt
]
\item A long wait can make a gradual transition feel sudden. Zoom in on the
training timeline, and the apparent jump becomes a response with measurable
width and shape.
\item In these runs, generalization was already under way before the dramatic
late rise: an early component delivered roughly one-third of the eventual
gain, and a sharper later component delivered the rest.
\item A single ``takeoff time'' names the most visible moment but misses the
route. A response kernel keeps track of when improvement arrives, how spread
out it is, and whether it comes in one wave or several.
\item Two waves in the learning curve do not, by themselves, prove two
mechanisms inside the model. Establishing that link requires interventions and
measurements of the model's intermediate states.
\end{itemize}
\end{questionbox}
...
\bibitem{kumar2024}
T. Kumar, B. Bordelon, S. J. Gershman, and C. Pehlevan, "Grokking as the
Transition from Lazy to Rich Training Dynamics," \emph{ICLR}, 2024.
\bibitem{lyu2024}
K. Lyu, J. Jin, Z. Li, S. S. Du, J. D. Lee, and W. Hu, "Dichotomy of Early and
Late Phase Implicit Biases Can Provably Induce Grokking," \emph{ICLR}, 2024.
\bibitem{prieto2025}
L. Prieto, M. Barsbey, P. A. M. Mediano, and T. Birdal, "Grokking at the Edge
of Numerical Stability," \emph{ICLR}, 2025.
\bibitem{li2026}
Z. Li, C. Fan, and T. Zhou, "Grokking in LLM Pretraining? Monitor
Memorization-to-Generalization without Test," \emph{ICLR}, 2026.
\end{thebibliography}
}
